...
## 適合すべき仕様: {docs/requirements の該当箇所。エージェントスライスは agent.md を必ず含む}
...
7. **エージェントスライス固有（agent.md 突き合わせ）** ※対象スライスがエージェントのとき
   - 使用ツールが agent.md「ツール一覧」の範囲内か。definition.py の値（システムプロンプトの完了条件・max_turns・タイムアウト）が agent.md と一致するか
