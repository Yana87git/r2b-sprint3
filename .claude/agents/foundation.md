...
- **トレース基盤**: すべての実行を `backend/traces/{run_id}.jsonl` に記録
  （agent.md の IPO 拡張表と同型 → 設計と実行を1対1で突き合わせられる）
- **タイムアウト2層構造**: 内側（エージェントの停止条件・無応答検知含む）< 外側（ジョブ層 `jobs.py` のフェイルセーフ）。
  ハングしても外側が必ず回収する
- 1ターンの疎通テストで動作確認

実際のエージェント（agent.md のツール・完了条件）は Phase 2 の `/build-loop`（エージェントスライス）で実装します。
...
スキルが完了したら、以下をユーザーに説明してください：
1. `app/agent/` の5ファイルの役割と、agent.md（エージェント設計書）との対応関係
...
3. 疎通テストで生成されたトレース（JSONL）を開き、input → decision → tool_use →
   observation → result の流れが agent.md の IPO 拡張表と同型であること
...
「トレースやタイムアウト2層の仕組みで不明な点はありますか？ この基盤の上に、Phase 3 で設計書（agent.md）のエージェントを実装します。」
...
- ✅ トレース基盤（backend/traces/・agent.md IPO 拡張表と同型）
...
build-loop（orchestrator）が設計書（01〜05・agent.md）から自分でスライスを分割して
