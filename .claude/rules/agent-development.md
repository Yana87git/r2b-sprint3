...
設計の正は `docs/requirements/agent.md`。

## 1. 設計が正（Design First）

- agent.md に無いツール・完了条件・ガードレールを実装しない。実装中に必要になったら、
  **先に agent.md を更新**してから実装する（設計と実装の乖離を作らない）
- `definition.py` の値（システムプロンプト・max_turns・タイムアウト）は agent.md の写し。
  片方だけ変えない
...
| エージェントループ（LLM の判断） | **評価シナリオ + トレース**（06 のエージェント評価シナリオを実行し、agent.md と突き合わせる） |
...
- agent.md の「してはいけない操作」は **PreToolUse hook / disallowed_tools でブロック**する。
...
- **内側**（`runner.py`）: エージェント自身の停止条件（agent.md の強制停止）。
