...
- **食い違いを見つけたら `/design-check` で判定する。** 設計書を引用してから
  「実装の誤り／設計の誤り／規定が無い／読み違い」を決める。**判定せずに片方を書き換えない**
  （テストの期待値を実装の挙動に合わせて直すのは、実装のバグを「仕様どおり」にする行為）。
  ツール一覧の食い違いは `.claude/scripts/spec-sync.mjs` が機械検出する（PostToolUse フック）
