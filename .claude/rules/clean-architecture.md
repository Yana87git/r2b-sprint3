...
| Business Logic（Agent） | `backend/app/agent/`（definition / tools / runner / jobs / trace） | エージェント実行（agent.md が設計の正） |
...
- [ ] 設計と実装の食い違いを `/design-check` の判定を経て扱ったか（引用つきで「実装の誤り／設計の誤り／規定が無い／設計が裁量として委ねている／読み違い」のどれかを決めてから直したか）
