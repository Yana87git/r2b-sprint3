...
`docs/requirements/agent.md` をもとに実装する。このスライスは**器とガードレールの基盤**を作る。
...
4. **トレース基盤**（JSONL・agent.md の IPO 拡張表と同型）の組み込み
...
- `docs/requirements/agent.md` が存在する（設計フェーズ完了）
...
├── app/agent/
│   ├── __init__.py
│   ├── definition.py    # システムプロンプト・停止条件の定数（agent.md Part 1 の写し先）
...
`docs/requirements/agent.md` の「完了条件・停止条件」を読み、
**強制停止の値（最大ターン数・タイムアウト）を設計から転記**して生成する。
未記載ならデフォルト値を使い、agent.md に追記するようユーザーに促す。

```python
# backend/app/agent/definition.py
"""エージェント定義: docs/requirements/agent.md Part 1 に対応する。

このファイルの値は設計書（agent.md）の写しであり、変更するときは agent.md 側も更新する。
"""

# ミッション（agent.md の「ミッション」を反映。build-loop のエージェントスライスが具体化する）
...
# ガードレール（agent.md の「ガードレール」に対応。build-loop のエージェントスライスが具体化する）
...
"""カスタムツール群: docs/requirements/agent.md「ツール一覧」と1対1で対応させる。

- ツールは入出力の決まった関数として実装し、単体テスト（TDD）の対象にする
- 副作用（write）を持つツールは agent.md のガードレールと突き合わせる
...
# build-loop（エージェントスライス）がここに agent.md のツールを追加していく
AGENT_TOOLS = [ping]
...
トレースの1レコードは **agent.md の「エージェントフロー（IPO拡張）」の表と同型**にする。
...
レコードは agent.md の IPO 拡張表（Input / 判断 / ツール実行 / 観察・Output）と同型。
...
class TraceRecorder:
    ...
    def record_tool_use(self, name: str, tool_input: dict) -> None:
        """ツール実行: 使用ツールが agent.md ツール一覧の範囲内かの検証に使う"""
    ...
    def record_result(self, stop_reason: str, *, num_turns: int | None = None,
        ...
        inactivity_timeout / outer_timeout のいずれか（agent.md 停止条件と対応付ける）"""
...
- 内側（このファイル）: INNER_TIMEOUT_S（実行全体）と INACTIVITY_TIMEOUT_S（無応答=ハング検知）。
  発火したらトレースに stop_reason を記録して整然と終了する。agent.md の「強制停止」に対応。
...
def _build_options() -> ClaudeAgentOptions:
    return ClaudeAgentOptions(
        ...
        # ガードレール（PreToolUse hook）は build-loop のエージェントスライスで agent.md をもとに追加する
    )
...
def read_progress(run_id: str, limit: int = 20) -> list[dict]:
    ...
    （agent.md「ユーザーから見た体験（実行中の見え方）」の実装手段）"""
...
- トレース: backend/traces/{run_id}.jsonl（agent.md IPO 拡張表と同型・進捗表示にも流用）
- 疎通テスト: Pass（ジョブ経由）
