---
name: build-loop
description: memory 駆動の自律実装ループ・オーケストレータ（Sprint 3）。memory.md からプランを作り、Web スライスとエージェントスライス（agent.md 由来）を TDD 実装→独立レビュー→修正→memory 記録で初版完成まで回す。継続改善も同じフローで行う。
...
- **エージェントスライス**: `docs/requirements/agent.md` に定義されたエージェント（AGENT-XX）。
...
- **設計が正**: agent.md に無いツール・完了条件を実装しない。実装中に必要が生じたら
...
1. 設計書を Read（存在するもののみ）:
   - `docs/requirements/01-request.md`, `02-requirement.md`, `03-spec.md`（モック含む）,
     `04-db.md`, `05-api-ipo.md`, **`agent.md`（エージェント設計）**
2. **設計書を実装スライスに分割する**（orchestrator 自身が判断）。分割基準:
   ...
   - **エージェントスライスを区別する**: agent.md の AGENT-XX は 1 体 = 1 スライス（大きければ
     「ツール群」「ループ+API接続」に分割可）。種別 `agent` を付ける
   - **依存順**: 基盤スライス（基本CRUD）→ **エージェントのツールが依存する API・テーブル
     （agent.md ツール一覧の最終列）を提供する Web スライス** → エージェントスライス の順に置く
...
1. `--change` の要求文と memory（§1 決定・§2 規約・§3 バックログ）を突き合わせ、影響スライスを特定。
   ...
   agent.md を読み、原因層（ツール / definition / 設計）を特定してから改修スライスを切る。
...
- **エージェントスライス**: [agent-slices.md](agent-slices.md) を Read し、その層順
  ...
  指示書には **agent.md の該当箇所（ツール一覧・完了条件・ガードレール）を必ず含める**
...
- テストケース（正常系 / 異常系）: {IPO・仕様・agent.md から導出}
- 参考にすべき既存実装: {path}
```
...
3. **エージェントスライスの definition / ガードレール層**（テスト対象外の設定層）:
   `Task(implementer)` に agent.md の該当箇所を渡して実装させる（agent-slices.md 参照）。
...
6. **エージェントスライスのみ・ミニ評価**（orchestrator 自身が Bash で実行）:
   agent.md「評価シナリオ」の正常系1本をジョブ経由で実行し、
...
1. **レビュー**: `Task(reviewer)` にこのスライスの**変更範囲**（対象ファイル群 / `git diff`）と
   ...
   **agent.md と、ミニ評価の run_id も渡す**。reviewer は read-only で**指摘のみ**返す。
2. **判定**:
   - 指摘ゼロ → 3 へ。
...
3. **BLOCKED が 0 かつ初版ビルドモード**なら「初版完成」と宣言し、次を案内する:
   - **エージェント評価**（CLAUDE.md Phase 3）: 06-scenario-test.md のエージェント評価シナリオを実行し、
     トレースを agent.md と突き合わせる。Fail は `/build-loop --change` で修正
...
- **agent.md に無いものを実装しない**（設計の更新が必要なら BLOCKED + 報告）。
