...
- agent.md「ツール一覧」の **1 行 = 1 ツール**（`@tool` 関数）
...
- `SYSTEM_PROMPT` を agent.md のミッション・完了条件から具体化
- `MAX_TURNS`・タイムアウト値を agent.md「強制停止」から転記（無応答 < 内側 < 外側の assert を維持）
- サブエージェント構成があれば SUBAGENTS として定義し options に渡す
  （各サブの使えるツール = agent.md 表のサブセット。SDK の正確な記法は
  https://code.claude.com/docs/en/agent-sdk/python で確認）
- ガードレール: agent.md「してはいけない操作」→ PreToolUse hook で block /
  「承認が必要な操作」→ 実行前確認の構造に
...
- progress（トレース末尾の decision / tool_use）を進捗表示に使う
  （agent.md「ユーザーから見た体験（実行中の見え方）」に対応）

### 層6: ミニ評価（orchestrator が Bash・レビュー前に必ず実施）

agent.md「評価シナリオ」の**正常系1本**をジョブ経由で実行する:
...
変更範囲に加えて必ず渡す:
- `docs/requirements/agent.md`（該当エージェントの節）
