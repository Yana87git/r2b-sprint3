...
> **本要求定義は、お題Aの前提（商社の引合書が Excel・PDF・Word・メールで届き、手で品目リストに転記されている）に基づく想定シナリオです。実在の企業・取引先ではありません。**
...
3. 営業事務が添付を開き、社内の品目リスト（Excel の共通フォーマット）に手で転記する
   ...
   - 1件の引合に、明細の Excel と仕様書の PDF のように複数の添付が届くことがある。営業事務はそれらを見比べながら1つの品目リストに転記している
...
| 形式 | 割合 | 実態 |
|------|------|------|
| Excel 添付 | 約4割 | 顧客ごとに独自フォーマットで、常時10種類以上が流通している |
| PDF 添付 | 約3割 | 顧客の基幹システム出力が大半。一部は手書き・FAX 由来のスキャン |
| メール本文に直書き | 約2割 | 箇条書きのことも、文章のこともある |
| Word 添付 | 約1割 | 文書として作られた依頼書。明細は文中の表に入っていることが多い |
...
1件の引合に投入されたものを「**入力**」と呼ぶ。入力は、行や値とは別の単位で状態を持つ。

**入力** … ファイル（.xlsx / .pdf / .docx）と、メール本文（投入された場合）を合わせた呼び方。メール本文もファイルと同じく1つの入力として数え、同じ状態を持つ。

| 入力の状態 | 意味 | 確定を止めるか |
|-----------|------|--------------|
| **読み取り済み** | 明細を1行以上抽出できた | 止めない |
| **読み取り済み（明細なし）** | 文字は読めたが、明細と判断できる部分がなかった | 止めない |
| **読み取り不可（理由つき）** | 判読できなかった | **止める**（除外するまで確定できない） |

#### 確定の条件と、「未確認を0にする」操作

- **確定できる条件は「未確認の行が0」かつ「読み取れなかった入力が0件」であること。**
...
2. **どこから読み取った値かがすぐ確認できる**
   ...
   | Word 添付 | 表の位置（何番目の表の何行目・何列目）、または何段落目か |
   | メール本文 | 本文の何行目か |
...
| 4 | 対応できる形式の範囲 | 受入 | ベテラン1名に依存 | **8件すべてを、ベテランに一度も聞かずに、1件10分以内（KPI1 と同じ取り方）で確定できる。誤りの見逃しは0件** | 4形式（Excel添付・PDF添付・Word添付・メール本文直書き）が各2回以上現れる計8件（うち2件は複数入力の案件）を、ベテラン以外の担当者が処理する。**うち2件は、その担当者が初めて見る書式とする**（課題3の核心が「新規顧客の見慣れない書式」であるため）。KPI8 と同じ8件を使う。8件それぞれに既知の誤りを1件ずつ仕込む（計8件）。確定後に出力された品目リストにその誤りがそのまま残っていた件数を「見逃し」と数える。見逃し0件を合格とする。誤りの仕込み方は `agent.md` の設計要件に従う |
...
|---|---------|--------------|------------------------|----------|------|
| 1 | 受け取る | 営業担当から転送された引合メールを開き、添付ファイル（Excel / PDF / Word）をシステムに投入する。本文に直接書かれている場合は、その本文を渡す | 同じように転送を受け取るが、ここから自分で読んで打ち込む作業が始まる | **スコア4**「開いて渡すだけでいい。ゼロから打ち込まなくていいのはありがたい」／顧客ごとに書式が違うので、投入する前に「この形式は扱えるのか」が分からず少し不安になる | 投入した時点で、何として読もうとしているか（Excel / PDF / Word / メール本文）が示される。扱えない場合はその場で「読み取り不可」と分かる |
...
Build フェーズはこの表を正とする。ここに無いものは Build で勝手に増やさない。

| 区分 | 指定 | 決めた理由 |
|------|------|-----------|
| 実行場所 | **ローカルのみ**。クラウドへのデプロイも CI も作らない | 評価期間が数日で、社外から預かった文書を扱うため（前提・制約3） |
| バックエンド言語 | **Python 3.12** | Claude Agent SDK（Python）を使うため |
| Python 環境 | **uv**。`uv run` を頭に付けて実行する（`python -m venv` は使わない） | 依存と仮想環境を1コマンドで揃えるため |
| Web フレームワーク | **FastAPI** | OpenAPI が自動で出るため、⑤ の19本を型付きで front に渡せる |
| データストア | **PostgreSQL**（Docker Compose で起動） | ④ の UUID 主キー・text＋CHECK 制約をそのまま書けるため |
| ORM・マイグレーション | **SQLAlchemy 2.0 ＋ Alembic** | ④ の制約（要確認でない値には原文と読み取り元がある、など）を DDL に落とせる |
| バックグラウンド実行 | **asyncio のタスク**（`asyncio.create_task`）。Celery・Redis は使わない | 利用者2名・ローカル実行のため。`agent.md` の2層タイムアウト（`runner.py` が内側、`jobs.py` が外側）を素直に書ける |
| フロントエンド | **Next.js 15**（App Router）。ポート 3000 | — |
| CSS 手法 | **CSS Modules ＋ 素の CSS 変数**。CSS フレームワークは使わない | ③ のデザイントークン（A案「検印」）を `mocks/mockup.html` の `:root` から**変換なしで持ち込める**。Tailwind にすると全トークンを書き直すことになる |
| UI コンポーネントキット | **使わない**（素の HTML 要素で作る） | 10画面すべてモックが素の HTML で成立している |
| 型生成 | OpenAPI → **orval**（時間が足りなければ手書きの fetch で代替してよい） | — |
| エージェント | **Claude Agent SDK（Python）**。モデルは `claude-sonnet-5` | `agent.md` の実行設定のとおり |
| テスト | backend: **pytest** ／ frontend: **Jest**（vitest ではない） | — |
| Lint・型 | **ruff**（Python）／ **eslint ＋ tsc**（TypeScript） | — |
| 認証 | **Scope 1 では実装しない**（固定の営業事務ユーザーで動かす） | 下記のとおり |

### スコープの縮小（9/13 の完成期限による判断）

公式スケジュールがお題Aの完成を 9/13（日）としているため、Scope 1 のうち次を今回の対象外とする。設計書からは削除せず、**未実装**として扱う。

| 対象外にするもの | 理由 |
|----------------|------|
| FUNC-12 ログイン／ログアウト（SCR-01） | ローカル単独実行で、利用者は固定1名でよい。エージェントの価値を示すのに寄与しない |
| SCR-07 行の修正・SCR-08 要確認一覧・SCR-10 読み取り不可 | 確認と確定の本流から外れる。読み取り不可の理由は SCR-02 の補足列に出す |
| **行**の除外・再実行・原本ダウンロード・問い合わせ文面（FUNC-09） | 同上。**入力の除外（⑤ #15）は残す**（これが無いと、一部が読めない案件を永久に確定できない） |
| orval による型生成 | 手書きの fetch で代替できる |

**残すもの**: 投入 → 読み取り → 品目リスト案の確認（読み取り元と確信度が見える） → 確定 → Excel 出力。**これがこのエージェントの価値そのもので、ここは削らない。**
