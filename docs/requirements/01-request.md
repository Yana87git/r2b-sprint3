...
- **確定できる条件は「未確認の行が0」かつ「読み取れなかったファイルが0件」であること。**
...
| 4 | 対応できる形式の範囲 | 受入 | ベテラン1名に依存 | **8件すべてを、ベテランに一度も聞かずに、1件10分以内（KPI1 と同じ取り方）で確定できる。誤りの見逃しは0件** | Excel添付・PDF添付・Word添付・メール本文直書きを各2件ずつ計8件、ベテラン以外の担当者が処理する。**うち2件は、その担当者が初めて見る書式とする**（課題3の核心が「新規顧客の見慣れない書式」であるため） |
