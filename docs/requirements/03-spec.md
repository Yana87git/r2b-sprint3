...
> **今回の実装範囲（① 6章 / 2026-09-13 時点）**: SCR-02・03・04・05・06・07・08・09・10 を作った。**未実装は SCR-01（ログイン）だけ**（認証は固定ユーザーで動かすため）。当初は SCR-07・08・10 も未実装としていたが、KPI4 の計測（誤りを直す）と読み取り不可の扱い（再実行・原本）に必要だったため実装した。
