...
|--------|-------|------|-------------|-------|
| SCR-01 | ログイン | 営業事務としてサインインする（Scope 1 は営業事務ロールのみ） | FUNC-12 | [mockup.html#SCR-01](./mocks/mockup.html#SCR-01) |
...
> **今回の実装範囲（① 6章 / 2026-09-13 時点）**: SCR-02・03・04・05・06・07・08・09・10 を作った。**未実装は SCR-01（ログイン）だけ**（認証は固定ユーザーで動かすため）。当初は SCR-07・08・10 も未実装としていたが、KPI4 の計測（誤りを直す）と読み取り不可の扱い（再実行・原本）に必要だったため実装した。行の除外（⑤ #13）も 2026-09-13 に実装した。
> - 読み取り不可の理由（判読不能／明細なし／タイムアウト／最大ターン数超過）は、**SCR-02 の補足列に出す**（SCR-10 の代わり）。原本ダウンロードと再実行の導線は作らない
> - SCR-05 の**入力の除外は作る**（これが無いと、一部が読めない案件を確定できない）。**行の除外は作らない**
> - 認証は作らず、固定の営業事務ユーザーで動かす
...
| 要素 | 説明 | 位置 | 対応機能(②) |
|------|------|------|-------------|
| ユーザーID入力 | 営業事務のユーザーID | 中央のフォーム | FUNC-12 |
| パスワード入力 | 入力は伏せ字で表示 | 中央のフォーム | FUNC-12 |
| 「ログイン」ボタン | primary | フォームの下 | FUNC-12 |
| メッセージ領域 | 認証失敗・ログイン期限切れを表示 | フォームの上 | FUNC-12 |

**操作とインタラクション**

| 操作 | UI要素 | 挙動 | 対応機能(②) |
|------|--------|------|-------------|
| IDとパスワードを入れてログインする | 「ログイン」ボタン | 認証に成功すると SCR-02 へ。アクセストークン（有効期限 60分）を保持する | FUNC-12 |
| パスワード欄で Enter | パスワード入力 | 「ログイン」ボタンと同じ | FUNC-12 |
...
|--------|---------|----------------|----------------|
| 認証失敗 | IDまたはパスワードが違う | メッセージ領域に「ユーザーIDまたはパスワードが違います」。どちらが違うかは出さない。パスワード欄だけ空にする | FUNC-12 |
| ログイン期限切れ | SCR-02〜10 のどこかで、トークンの有効期限（60分）が切れた状態で操作した | SCR-01 に戻し「ログインの有効期限が切れました。もう一度ログインしてください」。確認済み・修正は操作のたびに保存済みのため失われない（SCR-05） | FUNC-12 / 非機能・可用性 |
| 未入力 | IDかパスワードが空 | 「ログイン」ボタンを押せない状態にする | FUNC-12 |
...
| 要素 | 説明 | 位置 | 対応機能(②) |
|------|------|------|-------------|
| ヘッダー | 製品名・ログイン中のユーザー名・ログアウト（SCR-02〜10 共通） | 上端 | FUNC-12 |
| 「引合書を投入」ボタン | primary。SCR-03 へ | 右上 | FUNC-01 |
| 状況タブ | すべて／受付済み／読み取り中／読み取り不可／確認待ち／確定済み。各タブに件数を出す | 一覧の上 | FUNC-03 |
| 引合一覧表 | 列: 投入日時・件名（最初のファイル名。複数なら「ほか N 件」を添える。メール本文だけなら本文の1行目）・形式（複数なら「Excel＋PDF」のように並べる）・状況・補足（読み取り中は残り時間の目安、確認待ちは未確認の行数と読み取れなかった入力の数、確定済みは顧客回答待ちの行数、読み取り不可は理由。判読不能／明細なし／タイムアウト／最大ターン数超過）・投入者。投入日時の新しい順 | 中央 | FUNC-03 |
...
| 読み取り不可の案件がある | 状況が読み取り不可 | 状況ラベルを error で表示し、補足列に理由（判読不能／明細なし／タイムアウト／最大ターン数超過）を出す | FUNC-07 |
...
| 入力の件数 | 「入力 3件（ファイル2・メール本文1）／ 上限10件・合計50MB」のように、入力の件数と上限を出す。上限の判定はファイルとメール本文を合わせた入力単位である（② 非機能・性能） | 「投入する」ボタンの近く | FUNC-01 |
...
| 要素 | 説明 | 位置 | 対応機能(②) |
|------|------|------|-------------|
| 案件ヘッダー | 件名・投入した入力の一覧・投入日時 | 上部 | FUNC-03 |
...
| ケース | トリガー | 画面の挙動・表示 | 対応する要件(②) |
|--------|---------|----------------|----------------|
| 読み取り不可になった | 判読不能・明細なし・タイムアウト・最大ターン数超過 | 段階表示に「読み取り不可」を error で出し、理由を1行添えて「詳細を見る」→ SCR-10 | FUNC-07 |
...
**目的**: 抽出結果を「要確認」「確信が低い」「確信が高い」の3分類で並べ、重点的に見る行から確認し、未確認を0に、読み取れなかった入力を0件にして確定する（ジャーニー ステップ3・5）。
...
| 投入した入力の一覧 | 入力（ファイルとメール本文）ごとに、形式と読み取り結果（「11 行を読み取りました」、明細のない入力は「明細はありませんでした」。明細のない入力は確定を止めない）を並べる。メール本文も1行として表示する。読み取れなかった入力には理由（判読不能など）と「除外」ボタンを出す。除外した入力は「除外（手作業で補う）」と表示し、取り消せる | ヘッダーの下 | FUNC-07 / FUNC-06 |
...
| 確認のチェックボックス | **各行の左端（1列目）**。入れると確認済み、外すと取り消し。**確認済みの行は左端に帯を出す**（一覧でどこまで進んだかが見える）。確信が高い行は、その行の読み取り元を開くか抜き取りが min(3, N) に達するまで押せない（押せないときは「読み取り元を開くと確認できます」と出す） | 各行の左端 | FUNC-04 |
| 「修正」 | チェックボックスの隣（2列目）。SCR-07 を開く。**誤りを直すのが主な作業なので左に置く** | 各行の左（2列目） | FUNC-05 |
| 除外した行 | 品目表の最後に「除外した行（Excel に出力しない）」としてまとめ、グレー表示にして「除外」と示す。行の操作は「取り消し」だけにする | 品目表の最後 | FUNC-05 |
| 一括確認（全選択） | **確信が高い行のまとまりの見出しの左端にチェックボックスを置く**（全選択）。同じ見出しに「まとめて確認済みにする」と抜き取りの数「読み取り元を開いた行 1 / 3」を出す。**要確認・確信が低い行の見出しには置かない**（1行ずつ確認するのが要件のため）。表のヘッダーにも全選択は置かない | 確信が高い行の見出し | FUNC-04 |
| 「確定する」ボタン | primary。**未確認が0で、かつ読み取れなかった入力が0件のときだけ押せる**（確定を止める規則はこの1つで、理由の種類で扱いを変えない）。押せないときは理由と次の行動を横に出す（「未確認の先頭行へ」「除外すると確定できます」）。**サーバー側（⑤ #17）も同じ条件で拒否する** | 右下（スクロールしても見える位置） | FUNC-06 |
| 確定の確認ダイアログ | 「確定すると編集できません。要確認 2 行は『顧客回答待ち』として出力されます。除外した 1 行は出力しません」と「確定する」／「戻る」。除外した入力があるときは「仕様書.pdf は除外しました（手作業で補ってください）」も出す | 画面中央 | FUNC-06 |
...
| チェックボックスを入れる／外す | 確認のチェックボックス | 入れるとその行を確認済みにし、未確認の数を減らす。外すと取り消して未確認に戻す。操作のたびに保存する | FUNC-04 |
| 「修正」を押す | 「修正」 | SCR-07 を開く | FUNC-05 |
...
| 「除外」を押す（入力） | 投入した入力の一覧 | 読み取れなかった入力を「除外（手作業で補う）」にする。読み取れなかった入力が0件になれば、確定の条件の一方を満たす | FUNC-07 / FUNC-06 |
| 全選択のチェックボックス／「まとめて確認済みにする」を押す | 一括確認（全選択） | 抜き取りが min(3, N) 行に達していれば、確信が高い行をすべて確認済みにする。**足りないときも押せる状態にしておき、押したら次の行動を出す**（下のエラー表示） | FUNC-04 |
...
| 読み取れなかった入力がある | 複数の入力のうち一部が読み取り不可 | 入力の一覧でその入力を error で示し、理由と「除外」ボタンを出す。「確定する」を押せない状態にし、横に「読み取れなかった入力が N 件あります。除外すると確定できます」。未確認の行も残っているときは、両方の理由を並べて出す。**未確認のときと同じ扱いにする**（2026-09-13: 一時、この場合だけ押せるようにしてサーバーの 409 を出す実装にしたが、確定を止める規則が経路ごとに2通りになるため戻した。押せないときの次の行動は横の文言で足りる。「まとめて確認済みにする」を押せる形にしたのは、**次に何をすればよいかが画面から分からなかった**ためで、事情が違う） | FUNC-07 / FUNC-06 |
| 抜き取りが足りない | 読み取り元を開いた確信が高い行が min(3, N) 未満 | **全選択・「まとめて確認済みにする」は押せる状態にしておき、押したら**「あと 3 行の読み取り元を開いてください。値をクリックすると開きます」と出す（灰色のボタンと「0 / 3」だけでは、何をすればよいか分からないため）。**個別のチェックボックスは、読み取り元を開いていない確信が高い行では押せない**（「読み取り元を開くと確認できます」）。どちらもサーバー側が 409 `SAMPLING_NOT_ENOUGH` で拒否する | FUNC-04 |
...
| 原文と正規化後の値 | パネルには原文を表示する。正規化した値は、原文と正規化後の値を並べて見せる（例: 原文『10月末』／ 2026-10-21〜2026-10-31） | 対象の値の下 | FUNC-02 / FUNC-04 |
| 確信が低い理由 | 確信が低い値のとき、手がかりの記号と名前を出す（例「H2 列内の形式ずれ」）。複数あればすべて並べる | 対象の値の下 | FUNC-02 / FUNC-04 |
...
| 「{項目名}を要確認にする」 | 必須5項目それぞれに付く。**ラベルに項目名を入れる**（「型番を要確認にする」「納期を要確認にする」）。「要確認にする」とだけ書くと、項目が縦横に並ぶ中でどの項目に効くのか分からず、隣の項目に当ててしまう（2026-09-13 の計測で発生。⑥ TEST-05 参照） | 各入力欄の下 | FUNC-05 |
| 抽出時の値 | 修正前の値を、原文と正規化後の両方が分かる形で併記する（例「抽出時: 原文『10月末』／ 2026-10-21〜2026-10-31」「抽出時: 原文『3箱(1箱50本)』／ 数量3・単位箱」）。SCR-06 と同じ見せ方にそろえる | 各入力欄の下 | FUNC-02 / FUNC-05 |
...
| 「{項目名}を要確認にする」を選ぶ | 「{項目名}を要確認にする」 | その項目を「要確認」にし、入力欄を無効にする | FUNC-05 |
...
| 品目表（読み取り専用） | 編集できる見た目にしない（入力欄・行の操作・確認の列を出さない）。顧客回答待ちの行には印を付ける。**除外した行は表示しない**（確定ダイアログの「除外した 1 行は出力しません」と対応する） | 中央 | FUNC-06 |
| 「一覧へ戻る」 | SCR-02 へ | 下部左 | — |
...
**目的**: 読み取れなかった理由と読めた範囲を見せ、理由に応じた次の手（再実行、またはシステムの外で転記）に進める。案件のすべての入力が読み取れなかったときに使う（一部の入力だけが読み取れなかった案件は確認待ちに進み、SCR-05 で扱う）。

**UI要素**

| 要素 | 説明 | 位置 | 対応機能(②) |
|------|------|------|-------------|
| 理由の表示 | 判読不能／明細なし／タイムアウト／最大ターン数超過 を見出しで区別し、1行の説明を添える（明細なしは「読み取り不可 — 明細なし」「文字は読み取れましたが、品目の明細と判断できる部分が見つかりませんでした。添付を確認のうえ、明細を含むファイルを投入し直すか、今までどおり Excel に転記してください。」） | 最上部 | FUNC-07 |
| 読めた範囲 | 入力ごとに、どこまで読めてどこから読めなかったか（例「仕様書.pdf：明細の5行目まで読み取り、6行目以降は読めませんでした」）と、読めた行の一覧（読み取り専用） | 中央 | FUNC-07 |
| 案内文 | 判読不能・明細なしのとき「今までどおり Excel に転記してください」 | 読めた範囲の下 | FUNC-07 |
| 「再実行」ボタン | 理由がタイムアウト／最大ターン数超過のときだけ表示する（primary） | 下部右 | FUNC-07 / 非機能・可用性 |
| 「原本をダウンロード」 | 常に表示する。理由が判読不能・明細なしのときは primary、それ以外は secondary | 下部右 | FUNC-07 |
| 「一覧へ戻る」 | 常に表示する | 下部左 | — |

| 理由 | 表示するボタン |
|------|-------------|
| タイムアウト／最大ターン数超過 | 「再実行」（primary）＋「原本をダウンロード」＋「一覧へ戻る」 |
| 判読不能／明細なし | 「原本をダウンロード」（primary）＋「一覧へ戻る」 |
