...
| 品目表 | 列: 確認（済／未）・分類・品目名・型番・数量・単位・納期・備考・読み取り元。要確認の行と確信が低い行を上に寄せ、確信が高い行はその下にまとめる。複数のファイルに同じ品目があるときは、突き合わせずに別の行として並べる（② Out of Scope） | 中央 | FUNC-04 |
...
| 要素 | 説明 | 位置 | 対応機能(②) |
|------|------|------|-------------|
| 確定済みの表示 | 「確定済み・編集できません」 | 最上部 | FUNC-06 |
...
**（未確定）** 画面構成の合意後に、3案から選んで確定する。現在の `mockup.html` はグレースケールのワイヤーフレーム。
