...
| 4 | `/inquiries` | POST | 引合の投入。入力の形式判定と受付（FUNC-01） | 要 | 営業事務 |
| 5 | `/inquiries` | GET | 引合一覧。状況で絞り込み・並び替え（FUNC-03） | 要 | 営業事務 |
| 6 | `/inquiries/{inquiry_id}` | GET | 案件の状況と入力の一覧（FUNC-03 / FUNC-07） | 要 | 営業事務 |
| 7 | `/inquiries/{inquiry_id}/runs` | POST | エージェントの起動・再実行（FUNC-02 / FUNC-07） | 要 | 営業事務 |
...
| ステップ | Input | Process | Output | 対応API | 対応テーブル(④) |
|---------|-------|---------|--------|---------|----------------|
| 1 | ファイル（0〜10件）とメール本文（任意）（SCR-03） | 拡張子・サイズ・件数・合計サイズを検査し、形式を判定する。原本を保管し、入力を登録する | 案件ID・入力ごとの判定結果 | #4 | `inquiries`・`inquiry_inputs` |
| 2 | 案件ID | エージェントをバックグラウンドで起動する（`jobs.start_agent_job`） | 202 と run_id | #7 | `agent_runs` |
| 3 | run_id | 一定間隔で進み具合を取る。読み終えた入力の数と残り時間の目安を返す | 段階・残り時間（SCR-04） | #8 | `agent_runs`・`inquiry_inputs` |
| 4 | （エージェント内部） | 入力を読み、品目リスト案を保存し、完了条件を判定する | 案件が「確認待ち」または「読み取り不可」 | —（ツール） | `item_rows`・`item_values`・`value_clues`・`inquiry_inputs`・`inquiries` |
...
| ステップ | Input | Process | Output | 対応API | 対応テーブル(④) |
|---------|-------|---------|--------|---------|----------------|
| 1 | 案件ID | 品目リスト案を、要確認・確信が低い行を先にして返す。入力ごとの状態も返す | 行・値・読み取り元・手がかり（SCR-05） | #9・#6 | `item_rows`・`item_values`・`value_clues`・`inquiry_inputs` |
...
| 4 | 案件ID | 抜き取りが min(3, N) 行に達しているかを検査し、達していれば確信が高い行をまとめて確認済みにする | 未確認の数（不足なら 409） | #12 | `item_rows`・`item_values` |
...
| ステップ | Input | Process | Output | 対応API | 対応テーブル(④) |
|---------|-------|---------|--------|---------|----------------|
| 1 | 行ID・直した値（SCR-07） | 数量は数値、納期は (a)(b)(c)、必須5項目は値か要確認、を検査して保存する。保存できたらその行を確認済みにする | 更新後の行 | #10 | `item_rows`・`item_values` |
...
| ステップ | Input | Process | Output | 対応API | 対応テーブル(④) |
|---------|-------|---------|--------|---------|----------------|
| 1 | 案件ID | 未確認が0か、読み取れなかった入力が0件かを検査する。満たさなければ 409 と理由を返す | 確定の可否 | #17 | `item_rows`・`inquiry_inputs` |
...
| 2 | 案件ID（理由がタイムアウト／最大ターン数超過のとき） | 再実行の回数を検査（1回まで）し、エージェントを再起動する | 202 と新しい run_id | #7 | `agent_runs` |
...
| ステータス | 意味 | エラーコード |
|-----------|------|------------|
| 401 | ユーザーIDかパスワードが違う。**どちらが違うかは返さない**（SCR-01） | `AUTH_FAILED` |
| 403 | 無効化された利用者 | `USER_INACTIVE` |

### 引合の投入

- **Method**: POST ／ **Path**: `/api/v1/inquiries` ／ **目的**: 入力を受け付け、案件を作る
- **認証**: 要 ／ **必要権限**: 営業事務 ／ **対応テーブル(④)**: `inquiries`・`inquiry_inputs` ／ **対応フロー**: FLOW-02
...
- **Method**: POST ／ **Path**: `/api/v1/inquiries/{inquiry_id}/runs` ／ **目的**: 読み取りを始める。再実行も同じ入口
...
| 409 | 再実行できない（理由が判読不能・明細なし、またはすでに1回再実行した） | `RETRY_NOT_ALLOWED` |
...
- **認証**: 要 ／ **必要権限**: 営業事務 ／ **対応テーブル(④)**: `item_rows`・`item_values`・`value_clues` ／ **対応フロー**: FLOW-03・FLOW-04
...
| パラメータ | 必須 | 意味 |
|-----------|------|------|
| filter | No | `needs_confirmation` を指定すると要確認の項目だけ返す（SCR-08） |
| include_excluded | No | 除外した行も返すか（既定は返す。SCR-05 は最後にまとめて表示するため） |
...
| summary | 未確認の数・全行数・除外した行数・分類ごとの件数（画面の集計表示用に、その場で数えた値） |
...
**この API を呼ぶと、確信が高い行は抜き取りとして記録される**（同じ行は1回だけ）。一括確認の可否をサーバー側で判定するため（② FUNC-04）。
...
- **認証**: 要 ／ **必要権限**: 営業事務 ／ **対応テーブル(④)**: `item_rows`・`item_values` ／ **対応フロー**: FLOW-03
