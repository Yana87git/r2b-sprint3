...
| **9/27（日）** | **発表予備日** | 運営スケジュール表 2026-09-11 |
...
>
> **追記（2026-09-11）**: 運営スケジュール表に「発表会は、チームごとに指定された日程のうち、**原則として1日**にご参加いただきます」と明記された。**割り当てはチーム単位**なので、#0110_開発チーム10 での案内を待つ。交流会の対象チーム・集合方法も Slack で案内される。**9/27（日）が予備日**として新たに判明。

### 運営が示した公式スケジュール（2026-09-11 王さん投稿・参考値）

| 日付 | 取り組むこと／到達目標 |
|------|--------------------|
| 9/7（月） | Design に取り組む |
| **9/8（火）** | Design を進め、**7つの設計書を提出** |
| 9/9（水） | Build: お題A（エージェント構築）を開始 |
| 9/10〜9/12 | Build: お題A を進める |
| **9/13（日）** | Build: **お題A を完成** |
| **9/14（月）** | Build: **お題B（開発環境構築）を開始** |
| 9/15〜9/17 | Build: お題B を進める |
| **9/18（金）** | Build: **お題B を完成** |
| 9/19（土）〜9/20（日） | コードレビューを行い、エージェントについて理解を深める |
| 9/21〜9/23 | コードレビューと理解の深掘り。余裕があれば Learn のクイズ |
| 9/24（木） | 発表会の準備 |
| 9/25（金）・9/26（土） | 発表会 Day1・Day2（18:00〜20:00） |
| 9/27（日） | 発表予備日 |

> 王さんの投稿本文は「**（公式なスケジュールとしては）キャプチャのような進め方となっております**ので、ご参考までに！」であり、**絶対の締切ではない**。絶対なのは発表会だけ（§1）。
> ただし本文の主眼は明確で、「**今回はお題Bがかなり学びになる内容**で、作成しながら勉強できる部分なので**こちらにもぜひ時間を使えるようなスケジュール感**で進めていただけますと幸いです」とある。

**この表から読み取るべきは日付ではなく、重み付けの3点。**

1. **お題B に5日（9/14〜9/18）が充てられている。** 本計画書は当初これを M8・🟠推奨・「最悪ここを削る」扱いにしていたが、**その前提は誤り**。運営が明示的に時間を使ってほしいと言っており、発表項目にも入っている（§5 Phase 6）
2. **連休（9/19〜9/23）はコードレビューと理解の深掘りに充てられている。** 本計画書は連休に build-loop を置いており**向きが逆**。公式は Build を連休前に終える想定。Review 1日（M7）では足りないという §5 Phase 4 の懸念も裏付けられた
3. **公式は平日日中に作業できる前提**で組まれている。会社員である本人の可処分時間（平日は夜のみ＋週末）とは前提が違うため、**日付をそのまま追随することはできない**

> ⚠️ 2026-09-11 時点で、本人の進捗は公式より**約1週間ビハインド**（公式は「お題A を進める」段階、実際は Design 1本目の途中）。§4 のマイルストーンをこの重み付けに合わせて改訂するかは**要判断**。
...
発行したらポータルの **設定 → 外部連携** に `Yana87git` と `github_pat_...` を入力 → 接続を更新 → **古い Classic PAT を削除**（ポータルの URL は §8 を参照）。
...
| **ブートキャンプポータル（新）** | https://bootcamp-manage-production.up.railway.app/portal ／ **2026-09-11 にバージョンアップ。以後はこちらを参照する** |
| ブートキャンプアプリ（旧・参照しない） | ~~https://r2b-webapp.vercel.app/sprint3~~ ／ ~~`/assignments/sprint3`~~ |

> ⚠️ 新ポータルの**サブパス（Sprint3 タブ・提出物・設定→外部連携など）は未確認**。
> 旧 URL のパス構成がそのまま移ったとは限らないので、**ポータルのナビゲーションから辿ること**。
> 本計画書が「アプリの◯◯タブ」と書いている箇所は、新ポータル上での名称・位置を確認しながら読み替える。
