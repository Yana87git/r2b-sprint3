...
| **M1** | **9/9（水）** | Learn 完了 | 教材8本の読了 ＋ クイズ76問 | 🔴 必須 |
...
| **M6** | **9/22（火）** | エージェント評価 Pass | `06-scenario-test.md` の正常系・異常系を実行し、トレースと `agent-plan.md` の完了条件が一致 | 🔴 必須 |
| **M7** | **9/23（水）** | Review 完了 | `/r2b-review-sprint3` のガイド＋約80問（8割が記述）を通し、`docs/review/review-log.md` に記録 | 🟠 推奨 |
...
| ④ | **Fine-grained PAT を登録** | ❌ 登録済みPATは 2026-08-08 のもの（Classic の可能性が高い） |
| ⑤ | **提出先ブランチを設定** | ❌ 未設定 |
...
Build したプロダクトを題材に、C4 model の3階層（L1 全体像／L2 地図／L3 流れ）ごとに「ガイドを読む → そのレベルのクイズを解く」がブラウザアプリで提供される。約80問・**8割が記述式**で、Claude が実コードと突き合わせて 合格／もう一歩／再挑戦 の3値で採点する。
...
| # | リスク | 影響 | 対処 | 期限 |
|---|--------|------|------|------|
| R1 | **`ANTHROPIC_API_KEY` 未取得** | Slice 0-7 で完全に停止。**他社のキーでは代替不可**（`claude-agent-sdk` は Anthropic API のクライアント） | [Anthropic Console](https://console.anthropic.com/) で発行 → `backend/.env` に記載（git 管理外）。**費用は自己負担**。Console で Usage limit を設定しておく | **9/17（木）まで** |
...
- リポジトリ作成・push（`Yana87git/r2b-sprint3`）／R2B プラグイン v3.6.1（最新）
...
| # | やること | 理由 |
|---|---------|------|
| 1 | **Fine-grained PAT の発行・登録**（Phase 0 ④） | GitHub とアプリのブラウザ操作。PAT は他人に渡さない |
| 2 | **提出先ブランチの設定**（Phase 0 ⑤） | アプリのブラウザ操作。①の後でないと候補が出ない |
| 3 | **Anthropic API キーの発行**（R1・遅くとも 9/17） | 課金アカウントの登録が要る |
| 4 | **Learn 教材8本 ＋ クイズ76問**（M1・9/9まで） | 読んで理解することが目的。`~/r2b/r2b-docs/learn/02-Sprint3の全体像.md` から |
