...
> **改訂（2026-09-12 夜）**: 公式スケジュールがお題Aの完成を **9/13（日）** としているため、Build を前倒しした。
> M4（Foundation）は 9/12 夜に 0-1 → 0-2 → 0-3 → **0-7** まで（0-4 JWT は飛ばす）、翌 9/13 に 0-5（Next.js）と実装・評価を行う。0-6（orval）は飛ばし、手書きの fetch で代替する。
> あわせて Scope 1 を縮小した（① 6章）。未実装: 認証・SCR-07・SCR-08・SCR-10・行の除外・問い合わせ文面。実装する本流: 投入 → 読み取り → 確認 → 確定 → Excel 出力（入力の除外は残す）。
