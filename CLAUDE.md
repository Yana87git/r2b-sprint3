...
動かすための器であり、主役はエージェント（`backend/app/agent/`・設計の正は `docs/requirements/agent.md`）。
...
```
/build-loop
  ├─ ① memory.md + 設計書(01〜05, agent.md)を読む → プラン生成（memory §3 バックログ）
  ...
  │             → ミニ評価（正常系1本をジョブ実行しトレース×agent.md 突き合わせ）
...
Phase 3: エージェント評価   06-scenario-test.md のエージェント評価シナリオを実行し、
                          トレース（backend/traces/）を agent.md と突き合わせる（下記）
...
2. 各実行の `backend/traces/{run_id}.jsonl` を開き、agent.md と突き合わせる：
   - 使ったツールがツール一覧の範囲内か
   - ガードレール違反（禁止操作）がないか
   - 完了条件の「判定方法」どおりに終了したか（`*_timeout` / `max_turns` で終わっていないか）
   - フローが IPO 拡張表のスケッチから大きく乖離していないか
3. Fail は原因を 要件(02)・設計(agent.md)・実装 に切り分け、`/build-loop --change` で修正する
...
| **reviewer** | 🔵 実装範囲を独立レビュー（read-only・指摘のみ。agent.md 突き合わせ含む） | build-loop から Task |
...
| **トレース** | JSONL（`backend/traces/{run_id}.jsonl`） | agent.md の IPO 拡張表と同型。評価・進捗表示に使う |
...
│   │   │   ├── definition.py        #   システムプロンプト・完了/停止条件（agent.md Part 1 に対応）
│   │   │   ├── tools.py             #   カスタムツール群（agent.md ツール一覧と1対1）
...
│       └── entities/
├── docs/
│   ├── requirements/                # 設計ドキュメント（01〜06 + agent.md。build-loop の入力）
...
- **完了条件は機械判定可能に**（agent.md の「判定方法」をコードにする）
